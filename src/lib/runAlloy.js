...
*/
      ...
      var GET_DECISIONS_OPTIONS_SCHEMA = {
        viewStart: boundBoolean().default(false),
        scopes: boundArrayOf(boundString()).default([])
      };
      ...
      var validateOptions = function validateOptions(options) {
        var validate = boundObjectOf(GET_DECISIONS_OPTIONS_SCHEMA);
        var result = validate(options);
        var viewStart = result.viewStart,
          scopes = result.scopes;

        if (!viewStart && scopes.length === 0) {
          throw new Error("Invalid getDecisions command options parameter: " + "'viewStart' must be set to true or scopes must be defined.");
        }
        ...
        return {
          ...
          commands: {
            getDecisions: function getDecisions() {
              ...
              var _validateOptions = validateOptions(options),
                viewStart = _validateOptions.viewStart,
                scopes = _validateOptions.scopes; // Cloning scopes to avoid changing input options
